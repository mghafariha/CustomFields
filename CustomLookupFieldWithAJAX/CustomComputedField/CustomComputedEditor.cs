...
namespace CustomComputedField
{
    public class CustomComputedEditor : UserControl, IFieldEditor
    {
        ...
        public void OnSaveChange(SPField field, bool isNew)
        {
            ...
            string fieldValue = this.ddlFieldNameQuery.SelectedItem.Value;
            ...
            if (!IsPostBack)
            {
                ...
                using (SPWeb Web = SPContext.Current.Site.OpenWeb())
                {
                    ...
                    if (ddlListNameQuery.SelectedIndex != -1)
                    {
                        ...
                        ListItem fieldItem = ddlFieldNameQuery.Items.FindByValue(this.fieldNameValue);
                        if (fieldItem != null)
        ...
        private void BindDisplayColumns(SPList list)
        {
            ...
            foreach (SPField field in list.Fields)
            {
                ...
                {
                    ddlFieldNameQuery.Items.Add(new ListItem { Text = field.Title, Value = field.InternalName.ToString() });
                }
            }
